...
def run_task(model, langid_model, comet_model, task_name, batch_size, source_lang, steer_lang, cache_tag="base",use_beam_search=False):
    ...
    for i, (pred, ref,src) in enumerate(zip(pred, steer_ref,sources)):
        ...
        bleu_score = compute_bleu_score(model, ref, pred_post)
